...
## v18.4 照见信双钩子 (2026-06-11 更新)

### v18.4 核心变化

在照见信中植入两个 pull 钩子，让用户回来：

| 位置 | 意象 | 作用 |
|------|------|------|
| **裂缝之后** | 裂缝 × 光 | "你找到了一道，墙不会只裂一次" |
| **价值之后** | 账 × 还有没算的 | "这笔账算清了，可能还有别的规则在暗中收费" |

### 设计原则

- **位置**：放在注意力高的地方（前1/3和中间），不放最后
- **衔接**：AI 根据上下文自然过渡，融入叙述，不单独成段
- **语气**：像朋友说的话，不像广告

### 示例

**钩子1（裂缝之后）**：
> 这就是那条规则露出裂缝的地方。每一道裂缝，其实都是光能进来的入口——你已经找到了一道。墙不会只裂一次，下次它再裂开的时候，我们可以再来照一照。

**钩子2（价值之后）**：
> 这一笔账，你算清楚了。但你心里可能还有别的规则在默默收着费——下次想算的时候，随时回来。
